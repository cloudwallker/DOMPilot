...
并通过本地测试与 JSON Trace 展示决策、结果和耗时。
...
```console
uv run python -m http.server 8000 --bind 127.0.0.1 --directory tests/pages
```
...
模型名和 Key 必须指定。真实模型调用按服务计费，测试和默认 benchmark 不调用外部模型。
...
Wikipedia 搜索可作手动兼容性实验，不是 CI 验收条件。
...
可信调用方可传入 `verifier(page) -> passed | failed | unknown`；测试和 benchmark
...
测试只依赖本地 HTTP 页面与受控模型响应，覆盖非法输出、动态目标、节点替换、
旧引用、禁用控件、预算、终止条件与假 DONE。CI 配置在 Windows/Linux 运行测试和
benchmark。CI 不上传 Trace 工件；运行记录只保存在执行环境中。
