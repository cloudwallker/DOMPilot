...
项目按骨架与 fixtures、Snapshot 与人工操作、Agent Loop、动态动作约束、
停止与恢复、Trace 与 benchmark、文档与 CI 七个阶段构建。
...
测试覆盖严格输出、动态 Schema、引用失效、阻断边界、停止计数、异常记录和
独立成功验证。离线 benchmark 包含四项任务，各重复五次；只访问本地 HTTP
fixtures，不需要 API Key。真实 SDK 的响应和错误通过本地模拟服务测试。

Windows 与 Linux 的 GitHub Actions 均执行检查、测试和离线 benchmark。
CI 配置存在不代表已经运行通过，实际结果以仓库 Actions 页面为准。
本地已验证 Windows 上的隔离 Chrome Context；真实 LLM、第三方网站和性能
对比不属于离线验收结论。发布的 Trace 示例已替换日期、端口、标识和机器版本。

## 规模与设计边界

核心 Python 与 DOM 提取脚本约 1700 行（含空行，不计测试、fixtures、benchmark
...
仓库仅包含源码、虚构测试页面、锁文件、文档和脱敏 Trace 示例。`.env.example`
只含配置占位符；`.env`、实际 `runs/`、缓存、虚拟环境、浏览器下载和本地工具
均被忽略。CI 不上传运行 Trace 工件。真实任务及网页数据可能包含敏感内容，
